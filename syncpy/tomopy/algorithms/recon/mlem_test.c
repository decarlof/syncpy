...
void mlem_test(float* data, float* theta, float center, 
          int num_projections, int num_slices, int num_pixels, 
          int num_grid, int iters, float* recon, float* update) {
    ...
    for (t = 0; t < iters; t++) {
        ...
        sumay_ghost = (float *)calloc((num_slices*num_grid*num_grid), sizeof(float));
        simulate_ghost = (float *)calloc((num_projections*num_slices*num_grid), sizeof(float));
        ...
        for (k = 0; k < num_slices; k++) {
            for (n = 0; n < num_grid*num_grid; n++) {
                recon_scale = (sumay[i] / suma[n]);
                
                //if (t == 2) {
                    //if (recon_scale > 1e-5) {
                    //    update[i] = 1;
                    //}
                    //update[i] = recon_scale;
                //}
                //if (t > 0) {
                
                
                if (fabs(recon[i]*(recon_scale-1)) < 1e-4) {
                    update[i] = 0;
                }
                
                //update[i] = recon[i]*(recon_scale-1);
                //}
                //update[i] = fabs(recon[i]*(recon_scale-1));
                //
                //if (update[i] > 0) { 
                //    recon[i] *= recon_scale;
                //}
