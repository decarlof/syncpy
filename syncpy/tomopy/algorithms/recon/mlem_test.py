...
def _mlem_test(data, theta, center, num_grid, iters, init_matrix):
    ...
    update = np.ones((num_slices, num_grid, num_grid), dtype='float32')

    # Call C function.
    c_float_p = ctypes.POINTER(ctypes.c_float)
    c_int_p = ctypes.POINTER(ctypes.c_int)
    librecon.mlem_test.restype = ctypes.POINTER(ctypes.c_void_p)
    librecon.mlem_test(data.ctypes.data_as(c_float_p),
                  ...
                  update.ctypes.data_as(c_float_p))
    return init_matrix, update
